...
DefaultDocumentBuilder::make()
    ...
    ->margin($margin)
    ->text('Graphics Primitives', new TextOptions(
        x: $margin->left,
        y: PageSize::A4()->height() - $margin->top,
        fontSize: 24,
        color: $titleColor,
    ))
    ->text('This example combines several small graphics examples on one page: lines, stroked and filled rectangles, a rounded panel and custom paths.', new TextOptions(
        x: $margin->left,
        y: 760,
        width: 470,
        fontSize: 11,
    ...
    ->text('1. Line styles', new TextOptions(
        x: 55,
        y: 710,
        fontSize: 14,
        color: $accentColor,
    ...
    ->text('Thin neutral, medium accent, heavy alert.', new TextOptions(
        x: 270,
        y: 665,
        width: 220,
        fontSize: 10,
    ...
    ->text('2. Rectangles', new TextOptions(
        x: 55,
        y: 605,
        fontSize: 14,
        color: $accentColor,
    ...
    ->text('Stroke only', new TextOptions(x: 82, y: 500, fontSize: 10, color: $mutedColor))
    ->text('Fill only', new TextOptions(x: 195, y: 500, fontSize: 10, color: $mutedColor))
    ->text('Fill + stroke', new TextOptions(x: 298, y: 500, fontSize: 10, color: $mutedColor))
    ->text('3. Paths', new TextOptions(
        x: 55,
        y: 455,
        fontSize: 14,
        color: $accentColor,
    ...
    ->text('A closed triangle path can be filled and stroked; an open path stays stroked only.', new TextOptions(
        x: 320,
        y: 410,
        width: 170,
        fontSize: 10,
    ...
    ->text('4. Rounded rectangle', new TextOptions(
        x: 55,
        y: 280,
        fontSize: 14,
        color: $accentColor,
    ...
    ->text('Rounded rectangles reuse the same path machinery internally. That keeps the public API small while still covering a common page-decoration primitive.', new TextOptions(
        x: 95,
        y: 205,
        width: 360,
        fontSize: 11,
    ...
    ->text('Output: var/examples/graphics-primitives.pdf', new TextOptions(
        x: 95,
        y: 145,
        fontSize: 10,
        color: $mutedColor,
