...
final readonly class BengaliScriptTextShaper implements ScriptTextShaper
{
    public function supports(TextScript $script): bool
    {
        return $script === TextScript::BENGALI;
    }
    ...
    private function shapeCluster(
        ...
        if ($rephIndex !== null) {
            $glyphs[] = $this->shapeClusterGlyph(
                ...
                role: 'indic.reph',
                font: $font,
                unicodeText: $characters[$rephIndex] . '্',
            );
        }
    ...
    private function rephIndex(array $characters, int $baseIndex): ?int
    {
        ...
        if (($characters[0] ?? null) !== 'র' || !$this->isVirama($characters[1] ?? '')) {
            return null;
        }
    ...
    {
        return preg_match('/[\x{0995}-\x{09B9}\x{09DC}-\x{09DF}]/u', $character) === 1;
    }
    ...
    {
        return preg_match('/[\x{0981}-\x{0983}\x{09BC}]/u', $character) === 1;
    }
    ...
    private function isCombiningMark(string $character): bool
    {
        ...
        return preg_match('/[\x{0981}-\x{0983}\x{09BC}\x{09BE}-\x{09CD}\x{09D7}]/u', $character) === 1;
    }
}
