...
final readonly class DevanagariScriptTextShaper implements ScriptTextShaper
{
    ...
    {
        return $script === TextScript::DEVANAGARI;
    }
    ...
    private function shapeCluster(
        ...
        foreach ($characters as $index => $character) {
            ...
            $glyphs[] = $this->shapeClusterGlyph(
                ...
                role: $this->glyphNameForIndex($characters, $index, $baseIndex, $prefIndex),
                font: $font,
            );
        }
        ...
        if ($rephIndex !== null) {
            $glyphs[] = $this->shapeClusterGlyph(
                ...
                role: 'indic.reph',
                font: $font,
                unicodeText: $characters[$rephIndex] . '्',
            );
        }
     ...
     * @param list<string> $characters
     */
    private function glyphNameForIndex(array $characters, int $index, int $baseIndex, ?int $prefIndex): string
    {
        if ($index === $baseIndex) {
            return 'indic.base';
        }
    ...
    private function rephIndex(array $characters, int $baseIndex): ?int
    {
        ...
        if (($characters[0] ?? null) !== 'र' || !$this->isVirama($characters[1] ?? '')) {
            return null;
        }
    ...
    {
        return preg_match('/[\x{0915}-\x{0939}\x{0958}-\x{095F}]/u', $character) === 1;
    }
    ...
    {
        return preg_match('/[\x{0900}-\x{0903}\x{093C}]/u', $character) === 1;
    }
    ...
    private function isCombiningMark(string $character): bool
    {
        ...
        return preg_match('/[\x{0900}-\x{0903}\x{093A}-\x{094D}\x{0951}-\x{0957}\x{0962}-\x{0963}]/u', $character) === 1;
    }
}
