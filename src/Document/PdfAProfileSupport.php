    ...
    public static function for(Profile $profile): ?self
    {
        ...
        return match ($profile->name()) {
            ...
            'PDF/A-4e' => new self(
                'PDF/A-4e',
                true,
                'Supported for the current constrained PDF/A-4e scope with PDF 2.0 metadata, pdfaid:rev, no Info dictionary, no OutputIntent, the explicit Link/Text/Highlight/FreeText/RichMedia/3D annotation subset, the constrained AcroForm subset, and the simple optional-content group, configuration, membership and visibility-expression subset; SetOCGState push buttons, broader RichMedia/3D wiring and other engineering features remain blocked.',
                self::overrideCapabilityRules(self::baseCapabilityRules(
                    taggedPdf: false,
                    documentLanguage: false,
                    extractableUnicodeFonts: false,
                    ...
                    PdfACapability::NON_LINK_PAGE_ANNOTATIONS->value => new PdfACapabilityRule(
                        true,
                        false,
                        'Text, Highlight, FreeText, constrained RichMedia and constrained 3D annotations are allowed within the currently validated PDF/A-4e scope; popup-related objects, file-attachment annotations, richer RichMedia/3D dictionaries and broader engineering-specific page annotations remain blocked.',
                    ),
                    PdfACapability::ACRO_FORM_FIELDS->value => new PdfACapabilityRule(
                        true,
                        false,
                        'Text fields, checkboxes, radio buttons and choice fields are allowed within the currently validated PDF/A-4e form scope; push buttons, signature fields and broader engineering-specific interactive features remain blocked.',
                    ),
