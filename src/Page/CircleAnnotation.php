...
final readonly class CircleAnnotation implements AppearanceStreamAnnotation, PageAnnotation, TaggedPageAnnotation
{
    ...
    private function ellipsePathContents(): string
    {
        $kappa = 0.5522847498;
        $rx = $this->width / 2.0;
        $ry = $this->height / 2.0;
        ...
        return implode("\n", [
            $this->formatNumber($cx + $rx) . ' ' . $this->formatNumber($cy) . ' m',
            $this->formatNumber($cx + $rx) . ' ' . $this->formatNumber($cy + $oy) . ' '
                . $this->formatNumber($cx + $ox) . ' ' . $this->formatNumber($cy + $ry) . ' '
                . $this->formatNumber($cx) . ' ' . $this->formatNumber($cy + $ry) . ' c',
            $this->formatNumber($cx - $ox) . ' ' . $this->formatNumber($cy + $ry) . ' '
                . $this->formatNumber($cx - $rx) . ' ' . $this->formatNumber($cy + $oy) . ' '
                . $this->formatNumber($cx - $rx) . ' ' . $this->formatNumber($cy) . ' c',
            $this->formatNumber($cx - $rx) . ' ' . $this->formatNumber($cy - $oy) . ' '
                . $this->formatNumber($cx - $ox) . ' ' . $this->formatNumber($cy - $ry) . ' '
                . $this->formatNumber($cx) . ' ' . $this->formatNumber($cy - $ry) . ' c',
            $this->formatNumber($cx + $ox) . ' ' . $this->formatNumber($cy - $ry) . ' '
