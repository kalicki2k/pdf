...
mkdir -p "${OUTPUT_DIR}"
rm -f "${OUTPUT_DIR}"/pdf-a-3b-invalid-*.pdf

PDF_FILES="$(env UID="$(id -u)" GID="$(id -g)" docker compose run --rm php php /app/bin/generate-pdfa3b-negative-regression-fixtures.php /app/var/pdfa-regression)"

if [ -z "${PDF_FILES}" ]; then
    echo "No negative PDF/A-3b regression fixtures were generated." >&2
    exit 1
fi
