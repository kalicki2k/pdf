...
if ($argc !== 2) {
    fwrite(STDERR, "Usage: bin/generate-pdfa3a-negative-regression-fixtures.php <output-dir>\n");
    exit(1);
}
...
$baseDocument = DefaultDocumentBuilder::make()
    ...
    ->title('PDF/A-3a Negative Regression')
    ->author('kalle/pdf2')
    ->subject('Negative PDF/A-3a regression fixtures')
    ->language('de-DE')
...
$variants = [
    'pdf-a-3a-invalid-missing-catalog-markinfo.pdf' => static function (array $objects): array {
    ...
    'pdf-a-3a-invalid-missing-catalog-struct-tree-root.pdf' => static function (array $objects): array {
        return replaceFirstMatchingObject(
            $objects,
    ...
    'pdf-a-3a-invalid-missing-catalog-af-array.pdf' => static function (array $objects): array {
        return replaceFirstMatchingObject(
            $objects,
...
function replaceFirstMatchingObject(array $objects, callable $matcher, callable $mutator, string $label): array
{
    ...
    if (!$didMutate) {
        throw new RuntimeException(sprintf('Failed to mutate negative PDF/A-3a regression fixture for %s.', $label));
    }
