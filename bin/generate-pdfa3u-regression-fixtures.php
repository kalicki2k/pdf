...
if ($argc !== 2) {
    fwrite(STDERR, "Usage: bin/generate-pdfa3u-regression-fixtures.php <output-dir>\n");
    exit(1);
}
...
$fixtures = [
    $outputDir . '/pdf-a-3u-package.pdf' => createPdfA3uPackageFixture(),
];
...
function createPdfA3uPackageFixture(): Document
{
    return DefaultDocumentBuilder::make()
        ->profile(Profile::pdfA3u())
        ->title('PDF/A-3u Package Regression')
        ->author('kalle/pdf2')
        ->subject('PDF/A-3u package regression fixture')
        ->language('de-DE')
